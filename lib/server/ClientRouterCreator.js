...
var ClientRouterCreator = function (_BaseComponent) {
	...
	_createClass(ClientRouterCreator, [{
			...
			try {
				this.urlManager.config.controllerPath.map(function loopPath(controllerPath, index) {
					...
					if (/\./.test(controllerPath)) {
						sourcePath = (0, _RequireByFormat2.default)(controllerPath);
			...
			Object.keys(this.urlManager.rulesRegex).map(function loopRule(rule) {
				...
				if (ruleTarget === url) {
					(function () {
						...
						var ruleLangth = rule.split("\/").length - 1;
		...
		value: function fixSort(router) {
			...
			if (router.hasOwnProperty("/")) {
				(function () {
					var newRouter = Object.assign({}, router["/"]);
					var paramFirstRouter = {};
