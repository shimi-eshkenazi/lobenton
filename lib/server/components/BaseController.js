...
var BaseController = function (_BaseComponent) {
	...
	_createClass(BaseController, [{
		...
		value: function dispatchToStore(sourceState) {
			var reducerSrc = process.env.NODE_ENV === "dev" && !this.config.isStart ? "src/client/reducers" : "lib/client/reducers";
			var mainReducers = require(reducerSrc);
			...
			try {
				(function () {
					...
					if (_this2.layout) {
						...
						_this2.layout = process.env.NODE_ENV === "dev" && !_this2.config.isStart ? _this2.layout : _this2.layout.replace(/^src\//, "lib/");
						layoutSource = require(_this2.layout);
					} else {
						throw new Error("Render Error : Layout is not defined on constructor of controller : " + _this2.controller);
					}
					...
					(0, _reactRouter.match)({ routes: routes, location: location }, function matchHandle(error, redirectLocation, renderProps) {
						try {
							if (redirectLocation) {
								...
								throw new _NotFoundException2.default("Render Error : Route match error : Cannot find route '" + this.request.url + "'");
							} else {
