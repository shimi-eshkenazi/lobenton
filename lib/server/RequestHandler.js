...
var RequestHandler = function () {
	...
	_createClass(RequestHandler, [{
		...
		value: function loadAction(matchResult, controllerInstance) {
			try {
				...
				if (typeof action.method === 'string' && action.method.toUpperCase() !== this.request.method.toUpperCase()) {
					throw new _NotFoundException2.default("Http method Error : Cannot find action '" + matchResult.action + "' at '" + controllerInstance.controllerPath + "'; Url : " + this.request.url);
		...
		value: function fixArgs(action) {
			...
			try {
				var _ret = function () {
					var reqParams = _this.request.params || {};
					var reqBody = _this.request.body || {};
					var reqQuery = _this.request.query || {};
					var reqErrorObject = _this.request.errorObject || null;
		...
		value: function execError(data, error) {
			var re = new RegExp(this.config.defaultErrorController, "gi");
			...
			if (re.test(this.request.url)) {
				...
				if (controllerActionArray.length === 2) {
					...
					this.finalError(new _ErrorException2.default("Forward error : Cannot find pattern '" + controllerAction + "'"));
				}
			}
		}
	}, {
				...
				this.runPrecessChain(function processChainResult(errorMag) {
					try {
						...
						if (matchResult.controller !== null) {
							...
							var action = this.loadAction(matchResult, controllerInstance);
