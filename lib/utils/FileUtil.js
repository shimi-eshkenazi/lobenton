...
var FileUtil = function () {
	...
	_createClass(FileUtil, null, [{
		...
		value: function getFile(path) {
			var buffer = _fs2.default.readFileSync(path, "utf8");
		...
		value: function getFileList(path) {
			try {
				return _fs2.default.readdirSync(path, "utf8");
			} catch (e) {
				throw e;
			}
		}
		...
		value: function findControllerPath(expression) {
			var expressionArray = expression.split(".");
			var sourcePath = path.join(this.config.basePath, expressionArray[0].replace(/\/.+/g, ""));
