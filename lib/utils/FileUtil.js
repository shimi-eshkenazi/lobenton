...
function getControllerLayout(source) {
	var docExec = new RegExp("\\@layout\\:(((?!\\*).)*)\\*", "gi").exec(source);
	docExec = docExec || [];
...
var FileUtil = function () {
	...
	_createClass(FileUtil, null, [{
		...
		key: "getFile",
		value: function getFile(src, replaceSpace) {
		...
		key: "isFile",
		value: function isFile(src) {
			return _fs2.default.lstatSync(src).isFile();
		}
	}, {
		key: "isJsFile",
		value: function isJsFile(src) {
			return (/\.js(x|)$/gi.test(src)
			);
		}
	}, {
		key: "isWindows",
		value: function isWindows(src) {
		...
		key: "fixControllerLayout",
		value: function fixControllerLayout(target, source) {
			if (!source) {
